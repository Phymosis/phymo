...
$username = $_POST['usernameChal'];
$password    = "";
$errors = array();

// connect to the database
$db = mysqli_connect('localhost', 'phpmyadmin', '!27tVpb6jYs701zmv6', 'phplogin');
$token =  $_POST['token'];
...
if (!isset($_POST['usernameChal'])) {
        array_push($errors, "User required !");
        echo "No User";
}

if (!isset($_POST['token'])) {
        array_push($errors, "token required !");
	echo "No Token";
}

if (isset($_POST['usernameChal'])) {
  $username = mysqli_real_escape_string($db, $_POST['usernameChal']);
  $password = mysqli_real_escape_string($db, $_POST['passwordChal']);
  $password = $_POST['passwordChal'];
  if (empty($username)) {
  ...
  if (count($errors) == 0) 
  {
        $query = "SELECT * FROM challenges WHERE username='$username' AND password='$password' AND token='$token'";
	$results = mysqli_query($db, $query);
    if (mysqli_num_rows($results) == 1) 
    {
            $_SESSION['usernameChal'] = $username;
            ...
            $lvl_check_query = "SELECT level FROM `challenges` WHERE token='$token'";
            $result = mysqli_query($db, $lvl_check_query);
            $tmparr = $result->fetch_array();
            $chal = intval($tmparr[0]);

            if ($level === $chal) {
                $level = $level + 1;
                $db = mysqli_connect('localhost', 'phpmyadmin', '!27tVpb6jYs701zmv6', 'phplogin');
                $lvl_check_query = "UPDATE accounts SET level='$level' WHERE username='$username' OR email='$username';";
                $result = mysqli_query($db, $lvl_check_query);
            }
?>
...
<html lang="en" class="no-js dark-mode">
    <head>
        ...
        <title>Intro <?php echo $chal ?> | Defend the Web</title>
        ...
        <meta name="theme-color" content="#2d3b4f">
                    ...
                    <meta name="csrf-token" content="822cb8fe3c5842296c7ac1f5e4dab8563c1da8a8b33ea8821097a49d2d053480">
    ...
    <a href="/dashboard" class="logo">Defend the Web</a>
           ...
           <ul class="breadcrumb">
                            ...
                            <li><a href='/playground/intro'>Intro <?php echo $chal ?></a></li>
                    </ul>
    ...
    <div class="sidebar-content">
            ...
            action="/search"            method="POST"            enctype="multipart/form-data"
                    >
                        ...
                        <div class="input input--hidden " data-group="token">
                                                                                            ...
                                                                                            <input type="hidden" name="token" id="token" value="c93384d74bf75b97fdff5cb8b40c19413b10034126fa9c6eb501fc2b45a70956" maxlength="" placeholder="" class="u-full-width" />
                                                    </div>
            ...
            <button type="submit" class="button button--main right"><i class="fas fa-search"></i></button>
                    ...
                    <li class="label">
                                ...
                                Playground
                                            ...
                                            <div class="right small">3 of 60</div>
                                        </div>
                                                                                                </a>
                                            </li>
                                    </ul>
        ...
        <div class="row">
            <div class="eight columns">
                <div class="block">
                    <div class="block-heading block-heading--primary">
                                                    <div class="right">
            ...
            <div class="level-badge level-badge--complete">
            <div class="level-badge-emoji">
                👍
            </div>
            <div class="level-badge-message">
                Congratulations
                <div class="small">
                    <p>You have completed intro <?php echo $chal ?></p>
                                            <p><a class="button button--main" href="/index.php">Go Back Home</a></p>
                                        
                </div>
            </div>
        </div>
        <div class="block block--highlight block--share">
    ...
    <div class="row">
        <div class="eight columns offset-by-two level-form">
                        ...
                        <div class="block-content">
                            Last completed by <a href='/index.php'  data-profile-card='You'>You</a> <span title='31/10/2021 20:17 (UTC)'>seconds ago</span>
                        </div>
                    </div>
...
<?php
    ...
    {
        echo "Access Denied, Bad credentials, Try Again";
